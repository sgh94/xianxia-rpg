...
export async function getAllEventMetadata(): Promise<EventMetadata[]> {
  ...
  if (keys.length === 0) {
    return getMockEventMetadata(); // 실제 데이터가 없으면 모의 데이터 반환
  }
...
// 모의 이벤트 메타데이터 (실제 구현에서는 DB 값 사용)
function getMockEventMetadata(): EventMetadata[] {
  return [
    {
      id: 'mountain-cave',
      type: 'exploration',
...
// 이벤트 템플릿 가져오기
async function getEventTemplate(): Promise<string> {
  try {
    const promptPath = path.join(process.cwd(), 'prompts', 'event-template.txt');
    return fs.readFileSync(promptPath, 'utf-8');
  } catch (error) {
    console.error('이벤트 템플릿 파일 로드 오류:', error);
    throw new Error('Event template file not found');
  }
}
...
export async function generateEvent(
  ...
  try {
    ...
    const promptVariables = {
      fate: profile.traits.join(', '), // 실제로는 운명 정보 사용
      stats: profile.stats,
      traits: profile.traits,
      life: profile.life,
      maxLife: profile.maxLife,
      eventId: metadata.id,
      eventType: metadata.type,
      timeCost: metadata.timeCost,
      epReward: metadata.epReward,
      risk: metadata.risk,
      lifeDelta: metadata.lifeDelta || 0
    };
    
    // 프롬프트 생성
    const prompt = fillPromptTemplate(template, promptVariables);
    
    // Gemini API 호출
    const response = await generateTextWithGemini(prompt, 0.8);
    
    // JSON 파싱
    const result = extractJsonFromText(response);
    
    // 응답 형식 검증
    if (!result.narrative || !result.options || !Array.isArray(result.options)) {
      throw new Error('Invalid response format from Gemini API');
    }
    
    // 내러티브 저장
    await kv.set(`event:text:${sessionId}`, result.narrative);
